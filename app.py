...
since it's a RESTful API, the endpoints will be the same
but the requests that is sent to the endpoints will be different
"""
...
@app.route('/member', methods=['GET'])  # This returns all the members.
@protected
def get_members():
    db = get_db()
...
@app.route('/member', methods=['POST'])  # This adds a new member.
@protected
def add_member():
...
def edit_member(member_id):
    ...
    db = get_db()
    db.execute('update members set name = ?, email = ?, \
        level = ? where id = ?', [name, email, level, member_id])
    db.commit()

    member_cur = db.execute('select id, name, email,\
         level from members where id = ?', [member_id])
    member = member_cur.fetchone()
